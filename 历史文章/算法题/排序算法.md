...
function quickSortInPlace(arr, left = 0, right = arr.length - 1) {
  ...
  if (left < right) {
    ...
    quickSortInPlace(result, left, pivotIndex - 1);
    // 3. 递归排序右子区间（基准右侧，不包含基准）
    quickSortInPlace(result, pivotIndex + 1, right);
  }
 ...
 */
function quickSort(array: number[], compare: (a: number, b: number) => number) {
  ...
  while(stack.length) {
    ...
    if (pivotIndex - 1 > low) {
      // 当分区位置没有到最左边时，说明其左边还需要进行计算
      stack.push([low, pivotIndex - 1]);
    }
    if (pivotIndex + 1 < high) {
      // 当分区位置没有到最右边时，说明其右边还需要进行计算
...
// 取分区位置，并根据排序方式进行二分
function partition(array: number[], low: number, high: number, compare:(a: number, b: number) => number) {
  let left = low - 1;
